...
def ensure_inputs() -> None:
    ...
    missing = [path for path in (INPUT_IRCA, INPUT_SEDES) if not path.exists()]
    if missing:
...
def main() -> None:
    ...
    for output in (OUT_BASE_CSV, OUT_METADATA, OUT_RESUMEN_MUNICIPIO, OUT_RESUMEN_RIESGO, OUT_RESUMEN_ACUEDUCTO, OUT_CONTROL_CRUCE, OUT_SIN_GEO, OUT_VALIDAR_FECHAS):
    ...
    metadata = {
        ...
        "fuentes": [INPUT_IRCA.name, INPUT_SEDES.name],
