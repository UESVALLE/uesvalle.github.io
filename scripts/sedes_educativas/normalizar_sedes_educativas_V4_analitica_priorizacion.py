...
Entradas esperadas:
  ...
  data/sedes_educativas/raw/MEN_SEDES_EDUCATIVAS_PREESCOLAR_BÁSICA_Y_MEDIA_20260605.csv
...
INPUT_MEN_SEDES_ALT = RAW_DIR / "MEN_SEDES_EDUCATIVAS_PREESCOLAR_BÁSICA_Y_MEDIA_20260605.csv"
...
OUT_RESUMEN_GRUPO = CURRENT_DIR / "resumen_grupo_seguimiento.csv"
...
GROUP_ORDER = ["RUTINARIO", "PREVENTIVO", "PRIORITARIO", "BRECHA INFORMACION"]
GROUP_LABEL = {
    "RUTINARIO": "Sin riesgo y bajo",
    "PREVENTIVO": "Medio",
    "PRIORITARIO": "Alto e inviable",
    "BRECHA INFORMACION": "Sin dato / información incompleta",
}
...
def followup_group(risk: str) -> str:
    """Agrupa el riesgo IRCA en categorías operativas de seguimiento."""
    risk = normalize_risk(risk)
    if risk in ("SIN RIESGO", "BAJO"):
        return "RUTINARIO"
    if risk == "MEDIO":
        return "PREVENTIVO"
    if risk in ("ALTO", "INVIABLE SANITARIAMENTE"):
        return "PRIORITARIO"
    return "BRECHA INFORMACION"


def data_gap_flags(row: dict) -> list[str]:
    """Identifica brechas relevantes para trazabilidad y control de calidad."""
    flags = []
    if row.get("NIVEL_RIESGO_IRCA") == "SIN DATO":
        flags.append("SIN_DATO_IRCA")
    if int(row.get("ESTUDIANTES_ACTUAL", 0) or 0) <= 0 or row.get("FUENTE_MATRICULA_FINAL") == "SIN_FUENTE":
        flags.append("SIN_MATRICULA_VALIDADA")
    if row.get("FUENTE_COORDENADA") == "SIN_COORDENADA" or row.get("ESTADO_GEORREFERENCIACION") == "SIN_COORDENADAS":
        flags.append("SIN_COORDENADA")
    if row.get("ESTADO_CRUCE") in ("PENDIENTE", "VALIDAR_DUPLICADO"):
        flags.append("CRUCE_PENDIENTE")
    return flags


def data_quality(row: dict) -> str:
    """Clasifica la calidad analítica del registro para uso en filtros y priorización."""
    flags = data_gap_flags(row)
    critical_flags = {"SIN_DATO_IRCA", "SIN_MATRICULA_VALIDADA"}
    if not flags:
        return "ALTA"
...
def suggested_action(row: dict) -> str:
    """Genera una acción orientativa para gestión operativa."""
    risk = row.get("NIVEL_RIESGO_IRCA", "SIN DATO")
    students = int(row.get("ESTUDIANTES_ACTUAL", 0) or 0)
    zone = normalize_text(row.get("ZONA_IRCA", ""))
    gaps = set(data_gap_flags(row))

    if risk == "INVIABLE SANITARIAMENTE":
        return "Priorizar revisión técnica, validación en campo y articulación con sector educativo."
    if risk == "ALTO":
        return "Programar seguimiento operativo y verificar condiciones del sistema abastecedor."
    if risk == "MEDIO" and students >= 50:
        return "Mantener seguimiento preventivo por matrícula significativa."
    if risk == "MEDIO":
        return "Mantener seguimiento preventivo dentro de la programación regular."
    if "SIN_DATO_IRCA" in gaps:
        return "Gestionar actualización de la clasificación IRCA del sistema asociado."
    if "SIN_COORDENADA" in gaps:
        return "Priorizar georreferenciación de la sede educativa."
    if "SIN_MATRICULA_VALIDADA" in gaps:
        return "Validar matrícula con fuente institucional o Secretaría de Educación."
    if zone == "RURAL" and risk in ("BAJO", "SIN RIESGO"):
        return "Mantener seguimiento rutinario con control territorial rural."
    return "Mantener seguimiento rutinario."


def sanitary_priority_index(row: dict) -> float:
    """
    Índice analítico 0-100. No sustituye criterio técnico; ordena sedes para seguimiento.
    Componentes: riesgo IRCA, exposición estudiantil, concepto sanitario y brechas de información.
    """
    risk = row.get("NIVEL_RIESGO_IRCA", "SIN DATO")
    students = int(row.get("ESTUDIANTES_ACTUAL", 0) or 0)
    concept = normalize_text(row.get("CONCEPTO_SANITARIO", ""))
    ...
    risk_score = {
        ...
        "MEDIO": 40,
        "ALTO": 75,
        "INVIABLE SANITARIAMENTE": 100,
        "SIN DATO": 30,
    }.get(risk, 30)

    exposure_score = min(100, (math.log10(students + 1) / 4) * 100)
    if "DESFAVORABLE" in concept:
        concept_score = 100
    elif "REQUERIMIENTO" in concept:
        concept_score = 65
    elif concept:
        concept_score = 35
    else:
    ...
    gap_score = {"ALTA": 0, "MEDIA": 25, "BAJA": 55}.get(quality, 40)

    score = (0.55 * risk_score) + (0.25 * exposure_score) + (0.12 * concept_score) + (0.08 * gap_score)
    return round(min(100, max(0, score)), 2)


def analytic_priority_band(index: float) -> str:
    if index >= 75:
        return "MUY ALTA"
    if index >= 55:
        return "ALTA"
...
def ensure_inputs() -> None:
    ...
    if not INPUT_MEN_SEDES.exists() and not INPUT_MEN_SEDES_ALT.exists():
        missing.append(INPUT_MEN_SEDES)
    if missing:
...
def load_men_sedes() -> tuple[dict, dict]:
    ...
    """
    men_path = INPUT_MEN_SEDES if INPUT_MEN_SEDES.exists() else INPUT_MEN_SEDES_ALT
    men = pd.read_csv(men_path, dtype=object, encoding="utf-8-sig", sep=None, engine="python")
...
def consolidate(irca: pd.DataFrame, sedes: pd.DataFrame, men_by_code: dict, men_by_municipality: dict) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for _, row in irca.iterrows():
        ...
        out["GRUPO_SEGUIMIENTO"] = followup_group(out["NIVEL_RIESGO_IRCA"])
        out["RIESGO_IRCA_AGRUPADO"] = GROUP_LABEL.get(out["GRUPO_SEGUIMIENTO"], "Sin dato / información incompleta")
        out["BRECHAS_INFORMACION"] = "; ".join(data_gap_flags(out)) or "SIN_BRECHAS"
        out["TIENE_BRECHA_INFORMACION"] = "NO" if out["BRECHAS_INFORMACION"] == "SIN_BRECHAS" else "SI"
        out["CALIDAD_DATO_SEDE"] = data_quality(out)
        out["INDICE_PRIORIDAD_SANITARIA_ESCOLAR"] = sanitary_priority_index(out)
        out["NIVEL_PRIORIDAD_ANALITICA"] = analytic_priority_band(out["INDICE_PRIORIDAD_SANITARIA_ESCOLAR"])
        out["ACCION_SUGERIDA"] = suggested_action(out)
...
def create_summaries(base: pd.DataFrame):
    total_sedes = max(int(len(base)), 1)
    total_estudiantes = max(int(base["ESTUDIANTES_ACTUAL"].sum()), 1)
    ...
    risk["PORC_SEDES"] = (risk["SEDES"] / total_sedes * 100).round(2)
    risk["PORC_ESTUDIANTES"] = (risk["ESTUDIANTES"] / total_estudiantes * 100).round(2)

    group = (
        base.groupby("GRUPO_SEGUIMIENTO", dropna=False)
        .agg(
            RIESGO_IRCA_AGRUPADO=("RIESGO_IRCA_AGRUPADO", "first"),
            SEDES=("ID_SEDE_IRCA", "count"),
            ESTUDIANTES=("ESTUDIANTES_ACTUAL", "sum"),
            SEDES_CON_BRECHA=("TIENE_BRECHA_INFORMACION", lambda s: int((s == "SI").sum())),
            PRIORIDAD_PROMEDIO=("INDICE_PRIORIDAD_SANITARIA_ESCOLAR", "mean"),
        )
        .reindex(GROUP_ORDER, fill_value=0)
        .reset_index()
    )
    group["RIESGO_IRCA_AGRUPADO"] = group["GRUPO_SEGUIMIENTO"].map(GROUP_LABEL).fillna(group["RIESGO_IRCA_AGRUPADO"])
    group["PORC_SEDES"] = (group["SEDES"] / total_sedes * 100).round(2)
    group["PORC_ESTUDIANTES"] = (group["ESTUDIANTES"] / total_estudiantes * 100).round(2)
    group["PRIORIDAD_PROMEDIO"] = group["PRIORIDAD_PROMEDIO"].fillna(0).round(2)
    ...
    base_calc["BRECHA_INFO"] = base_calc["TIENE_BRECHA_INFORMACION"].eq("SI").astype(int)
    ...
    group_counts = pd.crosstab(base_calc["MUNICIPIO"], base_calc["GRUPO_SEGUIMIENTO"])
    group_counts = group_counts.reindex(columns=GROUP_ORDER, fill_value=0).reset_index()
    group_counts = group_counts.rename(columns={col: f"GRUPO_{col}" for col in GROUP_ORDER})
    ...
    totals = (
        ...
        .agg(
            ...
            SEDES_CON_BRECHA=("BRECHA_INFO", "sum"),
            PRIORIDAD_PROMEDIO=("INDICE_PRIORIDAD_SANITARIA_ESCOLAR", "mean"),
        )
    )
    municipality = totals.merge(counts, on="MUNICIPIO", how="left").merge(group_counts, on="MUNICIPIO", how="left")
    municipality["PORC_SEDES_ALTO_INVIABLE"] = (municipality["SEDES_ALTO_INVIABLE"] / municipality["SEDES_TOTAL"] * 100).round(2)
    municipality["PORC_SEDES_CON_BRECHA"] = (municipality["SEDES_CON_BRECHA"] / municipality["SEDES_TOTAL"] * 100).round(2)
    municipality["PRIORIDAD_PROMEDIO"] = municipality["PRIORIDAD_PROMEDIO"].fillna(0).round(2)
    ...
    aqueduct = (
        ...
        .agg(
            ...
            ESTUDIANTES_ALTO_INVIABLE=("EST_ALTO_INVIABLE", "sum"),
            SEDES_CON_BRECHA=("BRECHA_INFO", "sum"),
            PRIORIDAD_MAXIMA=("INDICE_PRIORIDAD_SANITARIA_ESCOLAR", "max"),
            PRIORIDAD_PROMEDIO=("INDICE_PRIORIDAD_SANITARIA_ESCOLAR", "mean"),
        )
        .sort_values(["SEDES_ALTO_INVIABLE", "ESTUDIANTES_ALTO_INVIABLE"], ascending=False)
    )
    aqueduct["PRIORIDAD_MAXIMA"] = aqueduct["PRIORIDAD_MAXIMA"].fillna(0).round(2)
    aqueduct["PRIORIDAD_PROMEDIO"] = aqueduct["PRIORIDAD_PROMEDIO"].fillna(0).round(2)

    return risk, municipality, aqueduct, group

# =============================================================================
# EXCEL DE CONTROL
# =============================================================================
def build_excel(base, control, risk, municipality, aqueduct, without_geo, validate_dates, validation_men, validation_matricula, group) -> None:
...
def main() -> None:
    ...
    risk, municipality, aqueduct, group = create_summaries(base)
    ...
    for output in (OUT_BASE_CSV, OUT_METADATA, OUT_RESUMEN_MUNICIPIO, OUT_RESUMEN_RIESGO, OUT_RESUMEN_GRUPO, OUT_RESUMEN_ACUEDUCTO, OUT_CONTROL_CRUCE, OUT_SIN_GEO, OUT_VALIDAR_FECHAS, OUT_ACUEDUCTOS_MAPA, OUT_VALIDACION_MEN, OUT_VALIDACION_MATRICULA):
    ...
    group.to_csv(OUT_RESUMEN_GRUPO, index=False, encoding="utf-8-sig", sep=";")
    ...
    metadata = {
        ...
        "grupo_seguimiento_sedes": {str(row["GRUPO_SEGUIMIENTO"]): int(row["SEDES"]) for _, row in group.iterrows()},
        "grupo_seguimiento_estudiantes": {str(row["GRUPO_SEGUIMIENTO"]): int(row["ESTUDIANTES"]) for _, row in group.iterrows()},
        "calidad_dato_sedes": {str(key): int(value) for key, value in Counter(base["CALIDAD_DATO_SEDE"]).items()},
        "brechas_informacion_sedes": int((base["TIENE_BRECHA_INFORMACION"] == "SI").sum()),
    }
    with OUT_METADATA.open("w", encoding="utf-8") as file:
        json.dump(metadata, file, ensure_ascii=False, indent=2)

    build_excel(base, control, risk, municipality, aqueduct, without_geo, validate_dates, validation_men, validation_matricula, group)
    ...
    print(f"  Sedes con brecha información   : {(base['TIENE_BRECHA_INFORMACION'] == 'SI').sum():,}".replace(",", "."))
    print()
